    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            if not config.testFlag:
                ...
                entity_embed = sess.run([trainModel.ent_embeddings])
                rela_embed = sess.run([trainModel.rel_embeddings])
                entity_embed = np.squeeze(np.array(entity_embed))
                rela_embed = np.squeeze(np.array(rela_embed))
                print 'entity embeddings: ', entity_embed.shape
                print 'relation embeddings: ', rela_embed.shape
                np.save('../data/entity_embed.npy', entity_embed)
                np.save('../data/rela_embed.npy', rela_embed)
            else:
